...
function specialtyCode(){const raw=val('specialty');if(globalThis.EducationCore&&typeof globalThis.EducationCore.normalizeSpecialtyCode==='function')return globalThis.EducationCore.normalizeSpecialtyCode(raw);return String(raw||'').trim().toUpperCase().replace(/^(?:PE|PΕ|ΠE|ΠΕ)/,'ΠΕ').replace(/^(?:TE|TΕ|ΤE|ΤΕ)/,'ΤΕ').replace(/^(?:DE|DΕ|ΔE|ΔΕ)/,'ΔΕ');}
function boolSelect(id){const v=val(id);return v==='yes'?true:v==='no'?false:null;}

function show(id,on){$(id).classList.toggle('hidden',!on);}
function pe86ComputerExcluded(){return val('role')==='educator'&&specialtyCode()==='ΠΕ86';}
...
const LANGUAGE_PAIR_OPTIONS={first:'language1',second:'language2',relatedFirst:['languageLevel1'],relatedSecond:['languageLevel2']};
function syncLanguageChoices(){return LanguagePairLock.sync(LANGUAGE_PAIR_OPTIONS);}
function languageChanged(){syncLanguageChoices();calculate();}
...
function data(){
 const role=val('role');
 return {role,specialty:specialtyCode(),eoppepAdultTrainer:val('eoppepAdultTrainer')==='yes',psychDegree:boolSelect('psychDegree'),psychLicense:boolSelect('psychLicense'),fppBefore1993:val('fppBefore1993')==='yes',psychMasterForFpp:boolSelect('psychMasterForFpp'),tertiaryDegree:boolSelect('tertiaryDegree'),careerQualification:val('careerQualification'),phd:val('phd'),master:val('master'),secondDegree:checked('secondDegree'),secondPhd:checked('secondPhd'),secondMaster:checked('secondMaster'),extraCredential:checked('extraCredential'),extraCredentialLabel:$('extraCredentialLabel').textContent.replace(/ — 1 μόριο$/,''),trainingSdeHours:val('trainingSdeHours'),trainingAdultHours:val('trainingAdultHours'),trainingThematicHours:val('trainingThematicHours'),thematicTrainingLabel:role==='psychologist'?'Επιμόρφωση στο αντικείμενο της Ψυχολογίας':'Επιμόρφωση στον Επαγγελματικό Προσανατολισμό',expSdeHours:val('expSdeHours'),expAdultHours:val('expAdultHours'),expFormalHours:val('expFormalHours'),expSdeMonths:val('expSdeMonths'),expAdultCounsellingMonths:val('expAdultCounsellingMonths'),language1:val('language1'),languageLevel1:val('languageLevel1'),language2:val('language2'),languageLevel2:val('languageLevel2'),computer:checked('computer'),unemploymentMonths:val('unemploymentMonths'),unemploymentExtraDays:val('unemploymentExtraDays'),threeChildren:checked('threeChildren'),singleParent:checked('singleParent'),manyChildren:checked('manyChildren'),disability:checked('disability')};
}
...
// External bindings: preserve the original per-control behavior without inline handlers.
const changeBindings={
  role:roleChanged,
  specialty:specialtyChanged,
  fppBefore1993:psychFppChanged,
  language1:languageChanged,
  language2:languageChanged
};
Object.keys(changeBindings).forEach(id=>{const el=$(id);if(el)el.addEventListener('change',changeBindings[id]);});
[
  'eoppepAdultTrainer','psychDegree','psychLicense','psychMasterForFpp','tertiaryDegree','careerQualification',
  'phd','master','secondDegree','secondPhd','secondMaster','extraCredential','languageLevel1','languageLevel2',
  'computer','threeChildren','singleParent','manyChildren','disability'
].forEach(id=>{const el=$(id);if(el)el.addEventListener('change',calculate);});
[
  'trainingSdeHours','trainingAdultHours','trainingThematicHours','expSdeHours','expAdultHours','expFormalHours',
  'expSdeMonths','expAdultCounsellingMonths','unemploymentMonths','unemploymentExtraDays'
].forEach(id=>{const el=$(id);if(el)el.addEventListener('input',calculate);});
const copyBtn=$('copyBtn');if(copyBtn)copyBtn.addEventListener('click',()=>copySummary(copyBtn));
const resetBtn=$('resetBtn');if(resetBtn)resetBtn.addEventListener('click',resetForm);
