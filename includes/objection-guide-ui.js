/*
 * Browser UI controller for odigos-enstasis.php.
 * The PHP page owns content/markup; this file owns conditional questions,
 * e-paravolo validation, deadline state and generated guidance.
 */
(function(global){
  "use strict";
...
function init(){
  const reason = document.getElementById('objectionReason');
  const recognition = document.getElementById('recognitionStatus');
  const submission = document.getElementById('submissionMode');
  const paravolo = document.getElementById('paravoloStatus');
  const paravoloCode = document.getElementById('paravoloCode');
  const guidanceBtn = document.getElementById('guidanceBtn');

  if(reason) reason.addEventListener('change', updateExtraQuestions);
  if(recognition) recognition.addEventListener('change', updateRecognitionDate);
  if(submission) submission.addEventListener('change', updateResubmissionInfo);
  if(paravolo) paravolo.addEventListener('change', updateParavoloUI);
...
global.ObjectionGuideUI = Object.freeze({
  showGuidance: showGuidance,
  updateExtraQuestions: updateExtraQuestions,
  updateRecognitionDate: updateRecognitionDate,
  updateResubmissionInfo: updateResubmissionInfo,
  updateParavoloUI: updateParavoloUI,
  validateParavoloCode: validateParavoloCode
});
