...
<section id="asepPeAcademic" class="card" data-component="asep-pe-academic" data-profile="eep" data-degree-required="false" data-specialty-id="branch" data-degree-id="degreeGrade" data-second-degree-id="secondDegree" data-phd-id="phd" data-msc-id="masters" data-language-id="asepLanguages" data-computer-id="computer" data-training-id="training" data-training-proof-id="trainingProof"><h2>3. Ακαδημαϊκά προσόντα</h2><p class="cap">Μέγιστο κατηγορίας Α: 120 μόρια.</p>
...
<section id="asepService" class="card" data-component="asep-service-criteria" data-warning-id="serviceWarning" data-subtotal-id="serviceSubtotal" data-warn-months="true"><h2>4. Εκπαιδευτική προϋπηρεσία</h2><p class="cap">Μέγιστο 120 μόρια. Μην δηλώνεις τον ίδιο μήνα σε περισσότερα από ένα πεδία.</p>
...
<script src="includes/service-calculations.js?v=3.20.26"></script><script src="includes/asep-service-controller.js?v=3.20.26"></script><script src="includes/social-calculations.js?v=3.20.26"></script><script src="includes/asep-social-criteria.js?v=3.20.26"></script><script src="includes/language-calculations.js?v=3.20.24"></script>
<script src="includes/asep-language-selector.js?v=3.20.24"></script><script src="includes/academic-calculations.js?v=3.20.30"></script><script src="includes/asep-pe-academic.js?v=3.20.30"></script><script src="includes/eep-eligibility-calculations.js"></script><script src="includes/training-proof.js?v=3.20.18"></script>
<script>
...
function render(){TrainingProof.syncAll();branchUI();const a=academic(),s=service(),c=social(),total=a.points+s.points+c.total;$('gradeWarning').classList.toggle('hidden',$('degreeGrade').value===''||a.degreeValid);$('academicSubtotal').textContent=fmt(a.points);$('totalPoints').textContent=fmt(total);$('degreePoints').textContent=fmt(a.degreePoints);$('academicPoints').textContent=fmt(a.points)+(a.rawPoints>120?' (όριο 120)':'');$('servicePoints').textContent=fmt(s.points)+(s.raw>120?' (όριο 120)':'');$('socialPoints').textContent=fmt(c.total);priority();const e=renderEligibility();return{a,s,c,e,total};}
function summary(r){const branch=$('branch'),branchText=branch.value?branch.options[branch.selectedIndex].textContent.trim():'κλάδος μη επιλεγμένος';return[`Υπολογισμός μορίων 2ΕΑ/2025 — ${branchText}`,`Βαθμός πτυχίου: ${fmt(r.a.degreePoints)}`,`Ακαδημαϊκά: ${fmt(r.a.points)}`,`Προϋπηρεσία: ${fmt(r.s.points)}`,`Κοινωνικά: ${fmt(r.c.total)}`,`ΣΥΝΟΛΟ: ${fmt(r.total)}`,`Άδειες/βεβαιώσεις: ${$('licenseResult').textContent}`,`Πρόταξη: ${$('priorityBox').textContent}${$('specialPriorityBox').classList.contains('hidden')?'':'\nΕιδική προτεραιότητα: '+$('specialPriorityBox').textContent}`,TrainingProof.summary('trainingProof')].filter(Boolean).join('\n');}
