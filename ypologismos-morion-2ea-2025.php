...
?>
<?php calculatorMainEnd(); ?><?php calculatorResultsStart(array('class' => 'results')); ?><?php calculatorCardStart(); ?><?php calculatorScoreHeader(array('value_id' => 'totalPoints', 'value_html' => '0,00', 'label' => 'συνολικά μόρια')); ?><?php calculatorResultRow(array('label_html' => 'Βαθμός πτυχίου', 'value_html' => '0,00', 'value_id' => 'degreePoints')); ?><?php calculatorResultRow(array('label_html' => 'Ακαδημαϊκά', 'value_html' => '0,00', 'value_id' => 'academicPoints')); ?><?php calculatorResultRow(array('label_html' => 'Προϋπηρεσία', 'value_html' => '0,00', 'value_id' => 'servicePoints')); ?><?php calculatorResultRow(array('label_html' => 'Κοινωνικά', 'value_html' => '0,00', 'value_id' => 'socialPoints')); ?><?php calculatorResultRow(array('label_html' => 'Άδειες / βεβαιώσεις', 'value_html' => '—', 'value_id' => 'licenseResult')); ?><?php calculatorResultMessage(array('id' => 'priorityBox', 'variant' => 'status', 'text' => 'Επίλεξε κλάδο για έλεγχο πρόταξης')); ?><?php calculatorResultMessage(array('id' => 'specialPriorityBox', 'variant' => 'success', 'class' => 'hidden', 'text' => '')); ?><?php calculatorActions(array(array('attrs' => array('type' => 'button', 'class' => 'secondary', 'id' => 'copyBtn'), 'html' => 'Αντιγραφή'), array('attrs' => array('type' => 'button', 'class' => 'secondary', 'id' => 'resetBtn'), 'html' => 'Καθαρισμός'))); ?><?php calculatorCardEnd(); ?><?php calculatorResultsEnd(); ?><?php calculatorColumnsEnd(); ?>
...
function syncEligibilityUI(){const b=$('specialty').value;const route=b==='PE25'?$('pe25Route').value:'';$('pe25RouteWrap').classList.toggle('hidden',b!=='PE25');const key=b+'|'+route;if(key===eligibilityUiKey)return;eligibilityUiKey=key;const host=$('licenseRequirements'),intro=$('licenseIntro');host.innerHTML='';if(!b){intro.textContent='Επίλεξε κλάδο για να εμφανιστούν τα απαιτούμενα πρόσθετα επαγγελματικά δικαιολογητικά.';return;}const req=EEPEligibility.getRequirements(b,route);if(req.routeRequired&&!req.route){intro.innerHTML='<strong>'+req.label+':</strong> '+req.note;return;}if(req.items.length===0){intro.innerHTML='<strong>'+req.label+':</strong> '+req.note;return;}intro.innerHTML='<strong>'+req.label+(req.routeLabel?' — '+req.routeLabel:'')+':</strong> έλεγξε ότι διαθέτεις όλα τα παρακάτω δικαιολογητικά.';req.items.forEach((item,i)=>{const row=document.createElement('div');row.className='checkrow';const id='licenseReq'+i;row.innerHTML='<input type="checkbox" id="'+id+'" data-license-id="'+item.id+'"><label for="'+id+'">'+item.label+'<small>Υποχρεωτικό πρόσθετο δικαιολογητικό — δεν προσθέτει μόρια.</small></label>';host.appendChild(row);});}
function eligibility(){syncEligibilityUI();const b=$('specialty').value,route=b==='PE25'?$('pe25Route').value:'';return EEPEligibility.evaluate(b,route,selectedEligibilityIds());}
