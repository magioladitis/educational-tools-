...
check('automatic proposal controls visible', 'id="autoAllocateRemaining"' in out and 'Αυτόματη πρόταση κάλυψης' in out and 'id="clearAllocationRows"' in out and 'Μέγιστη δυνατή κάλυψη → Α΄/ειδική → Β΄ → Γ΄' in out)
...
check('Religion is now available through PE02 C assignment when Ethics is unresolved', bool(slot_selects) and any('Θρησκευτικά' in block for block in slot_selects))
