...
out=render(base)
check('allocation tab active', re.search(r'<button[^>]*data-staffing-tab="allocation"[^>]*aria-selected="true"[^>]*>4\. Κατανομή μαθημάτων</button>',out) is not None)
check('allocation card rendered', '<h2>4. Κατανομή μαθημάτων</h2>' in out)
check('automatic proposal note explains editable optimization', 'Πρόταση κάλυψης — πάντα επεξεργάσιμη.' in out and 'επιδιώκει πρώτα τη μέγιστη δυνατή κάλυψη' in out)
...
check('hidden no-eligible slots are explained without removing their uncovered hours', 'χωρίς επιλέξιμο εκπαιδευτικό' in (out+'\n'+UI.read_text(encoding='utf-8')) and 'Οι ώρες τους εξακολουθούν να υπολογίζονται στις ακάλυπτες ώρες.' in UI.read_text(encoding='utf-8'))
