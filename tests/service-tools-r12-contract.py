...
check('hours service 50-year cap', 'Math.min(50, nonNegativeInteger(years))' in hours)
...
hours_ui = (ROOT / 'includes' / 'teaching-hours-ui.js').read_text(encoding='utf-8')
check('hours UI years max 50', 'id="serviceYears" type="number" min="0" max="50"' in page_hours)
...
check('hours UI hard-clamps years while typing', "el.id === 'serviceYears'" in hours_ui and 'clampBoundedIntegerInput(el, 50)' in hours_ui)
check('hours UI hard-clamps months while typing', "el.id === 'serviceMonths'" in hours_ui and 'clampBoundedIntegerInput(el, 11)' in hours_ui)
check('hours UI hard-clamps days while typing', "el.id === 'serviceDays'" in hours_ui and 'clampBoundedIntegerInput(el, 29)' in hours_ui)
...
check('salary service 50-year cap', 'Math.min(50, nonNegativeInteger(years))' in salary)
...
check('salary UI years max 50', 'id="serviceYears" type="number" min="0" max="50"' in page_salary)
check('salary UI months max 11', 'id="serviceMonths" type="number" min="0" max="11"' in page_salary)
check('salary UI hard-clamps years while typing', "el.id === 'serviceYears'" in salary_ui and 'clampBoundedIntegerInput(el, 50)' in salary_ui)
check('salary UI hard-clamps months while typing', "el.id === 'serviceMonths'" in salary_ui and 'clampBoundedIntegerInput(el, 11)' in salary_ui)
...
check('salary suspension UI clamp', 'clampSuspendedInputs' in salary_ui and "el.id === 'suspendedYears' || el.id === 'suspendedMonths'" in salary_ui)
check('salary suspension GLK source', '2/31029/ΔΕΠ/06-05-2016' in page_salary)

tools_proc = subprocess.run(['php', str(ROOT / 'ergaleia.php')], capture_output=True, text=True)
check('toolbox renders', tools_proc.returncode == 0)
tools = tools_proc.stdout
tools_js = (ROOT / 'assets' / 'tools-directory.js').read_text(encoding='utf-8')
toolbox_card_count = len(re.findall(r'class="tool-card"', tools))
check('toolbox has cards', toolbox_card_count > 0)
check('toolbox hero count matches cards', f'{toolbox_card_count} διαθέσιμα εργαλεία' in tools)
check('toolbox results count is maintained dynamically', "'Εμφανίζονται ' + visible + ' εργαλεία.'" in tools_js)
...
check('toolbox last number matches card count', f'class="tool-number">{toolbox_card_count}<' in tools)
