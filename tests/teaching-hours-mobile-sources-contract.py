...
check('mobile initializes sources collapsed', 'initialiseResponsiveSourceCards' in common and "max-width: 650px" in common and '(hover: none) and (pointer: coarse)' in common)
check('desktop expansion is inherited from shared source component', "$config['desktop_expanded'] = true" in source and "$config['open'] = false" in source)
...
if proc.returncode == 0:
    ...
    check('rendered source card defaults closed to avoid mobile flash-open', '<details class="edu-disclosure edu-source-card__details" open>' not in html)
