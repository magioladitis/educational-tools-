...
check('no programmatic requestSubmit path', 'form.requestSubmit()' not in text)
check('native submit is armed only by explicit click action', "button.addEventListener('click'" in text and "data-staffing-request-action" in text and "form.dataset.explicitRequest='1'" in text)
...
check('explicit calculation reuses one workload model for matrix and specialty catalogue', '$teachingModel = teachingWorkloadModelForProfile($profile);' in text and 'schoolProfileWorkloadMatrix($profile, $teachingModel)' in text and 'staffingUiPersonnelSpecialtyOptions($displayMatrix, $teachingModel)' in text)
...
check('backend reports over-limit French clearly', 'Στην Α΄ τάξη οι ομάδες «Γαλλικά» (3) δεν μπορούν να είναι περισσότερες από τα 2 κανονικά τμήματα της ίδιας τάξης.' in invalid_page)
