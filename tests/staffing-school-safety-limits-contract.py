...
base_inputs=re.findall(r'<input(?=[^>]*data-basic-section=)(?=[^>]*max="120")(?=[^>]*inputmode="numeric")(?=[^>]*step="1")(?=[^>]*type="number")[^>]*>', get_html)
check('all 12 base section inputs carry native max 120 and numeric input hint', len(base_inputs)==12)
