...
check('page uses semantic timetable row markup', 'timetable-course-row' in UI and 'timetable-course-title' in UI and 'timetable-course-meta' in UI and 'timetable-course-detail' in UI)
check('timetable detail labels are explicit', 'Ενότητα:' in UI and 'Προϋπόθεση:' in UI and 'timetable-detail-label' in UI)
check('render resolves specialty explicitly', "var specialty = specialtyField.hidden ? '' : specialtySelect.value;" in UI)
