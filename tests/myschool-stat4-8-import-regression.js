...
const csvText=headers.join(';')+'\n'
 ...
 +'="2401010";1ο ΗΜΕΡΗΣΙΟ ΓΥΜΝΑΣΙΟ ΚΕΡΚΥΡΑΣ;100003;ΜΠΟΥΝΙΑΣ;ΓΕΩΡΓΙΟΣ;ΠΕ03.50;;Όχι;Όχι;23;23;0';
...
check('three placements two schools',registry.placement_count===3&&registry.school_count===2);
check('transient source AM only used for aggregate unique count',registry.unique_people_count===3&&!Object.prototype.hasOwnProperty.call(registry.people[0],'source_am'));
...
const eaePeople=imp.forSchool(registry,'2401010');
check('PE03.50 EAE row preserved from stat4_8',eaePeople.length===1&&eaePeople[0].specialty_code==='ΠΕ03.50');
check('PE03.50 EAE 23 hours preserved',eaePeople[0].required_teaching_hours===23&&eaePeople[0].available_here_hours===23&&eaePeople[0].assigned_external_hours===0);
...
(async()=>{
 try{
   ...
   check('direct ZIP import',fromZip.placement_count===3&&fromZip.source_inner_file==='stat4_8-test.csv');
   ...
   check('session load',imp.loadSession(fake).placement_count===3);
