...
check('compact help replaces version card', 'ⓘ Βοήθεια' in get and 'Πώς χρησιμοποιείται το εργαλείο' in get and '<h2>Τι κάνει αυτή η έκδοση</h2>' not in get)
...
check('automatic proposal is exposed but not presented as official placement', 'Αυτόματη πρόταση + χειροκίνητες αλλαγές' in get and 'Η αυτόματη λειτουργία δημιουργεί <strong>πρόταση</strong>, όχι διοικητική πράξη τοποθέτησης.' in get)
check('official vacancy disclaimer exists', 'δεν αποτελούν από μόνα τους επίσημη πράξη προσδιορισμού λειτουργικών κενών' in get)
catalog=(ROOT/'includes'/'tools-catalog.php').read_text(encoding='utf-8')
check('ergaleia catalog links simulator', 'ypologismos-didaktikon-anagkon.php' in catalog and 'tools-catalog.php' in ERG.read_text(encoding='utf-8'))
check('tool card number 32 exists', "'number' => 32" in catalog and "'href' => 'ypologismos-didaktikon-anagkon.php'" in catalog)
...
check('Technical top-priority summary hidden', 'ώρες κοινής κορυφαίας Α΄/Β΄/Γ΄' not in g and 'ώρες αποκλειστικής κορυφαίας Α΄/Β΄/Γ΄' not in g)
...
check('crafted over-limit split is reported', 'Στην Α΄ τάξη τα τμήματα με πάνω από 21 μαθητές (5) δεν μπορούν να είναι περισσότερα από τα 2 κανονικά τμήματα.' in gi)
...
check('crafted over-limit language group is reported clearly', 'Στην Α΄ τάξη οι ομάδες «Γαλλικά» (3) δεν μπορούν να είναι περισσότερες από τα 2 κανονικά τμήματα της ίδιας τάξης.' in gli)
...
check('frontend does not execute personnel auto allocation', re.search(r"(?<!['\"])personnelWorkloadRosterPlan\s*\(", text) is None and re.search(r"(?<!['\"])personnelWorkloadEvaluatePerson\s*\(", text) is None)
check('visible calculation buttons are not native submit controls', 'type="submit" name="staffing_action"' not in text and text.count('data-staffing-request-action=') == 4)
check('centralized explicit request gate exists', 'function installExplicitRequestGate(form)' in text and text.count("form.addEventListener('submit'") == 1)
