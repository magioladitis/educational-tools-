...
check('live auto balance uses second specialty eligibility', 'allocationBestAssignment(allocationPeopleData[pid],slot)' in SRC and "specialty_source==='secondary'" in SRC)
check('automatic proposal respects B limit', 'b_remaining_hours:Math.max(0,10-bHours)' in SRC and "if(match.priority==='B') available=Math.min(available,personState[pid].b_remaining_hours);" in SRC)
