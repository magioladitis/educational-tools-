...
check('included in gross', 'grossForNet = result.basicGrossSalary + familyAllowance + positionAllowance + remoteAllowance' in page)
check('gross passed to net engine', 'grossMonthly: grossForNet' in page)
check('separate result row', 'remoteAllowanceResult' in page)
check('gross subtotal result', 'grossForNetResult' in page)
check('reset clears checkbox', "byId('remoteAreaAllowance').checked = false" in page)
