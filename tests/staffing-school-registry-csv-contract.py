...
check('current supported school types remain enabled', re.search(r'<option value="gymnasio"[^>]*>Ημερήσιο Γυμνάσιο</option>',html) is not None and re.search(r'<option value="esperino_gymnasio"[^>]*>Εσπερινό Γυμνάσιο</option>',html) is not None and re.search(r'<option value="gel"[^>]*>Ημερήσιο Γενικό Λύκειο</option>',html) is not None and re.search(r'<option value="esperino_gel"[^>]*>Εσπερινό ΓΕΛ</option>',html) is not None)
