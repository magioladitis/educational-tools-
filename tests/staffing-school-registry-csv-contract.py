...
check('200 basic-section safety limit is advertised in schema and UI', 'maxBasicSections:MAX_BASIC_SECTIONS' in js and 'έως <strong>200 βασικά τμήματα συνολικά</strong>' in page and 'max="200"' in page)
...
const valid200=csv.validateRegistry([{school_id:'a',school_code:'1',general_a:100,general_b:50,general_c:50}]);
const over201=csv.validateRegistry([{school_id:'a',school_code:'1',general_a:100,general_b:50,general_c:51}]);
...
check('exact 200 basic sections accepted by registry validator', '"max":200' in n.stdout and '"valid200":true' in n.stdout)
check('201 basic sections rejected by registry validator', '"over201":false' in n.stdout and '"overTotal":201' in n.stdout)
